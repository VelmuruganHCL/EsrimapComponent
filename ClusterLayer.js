  ...
  PopupTemplate, GraphicsLayer
    ...
    return declare([GraphicsLayer], {
      constructor: function (options) {
        ...
        this._layerArray = options.layerArray;
        this._zoomEnd = null;
        this.map;
      ...
      _layerCluster: function () {

        this._clusterResolution = this.map.extent.getWidth() / this.map.width; // probably a bad default...
        this.clear();
      ...
      add: function (p) {
        ...
        if (!clustered) {
          // this._layerArray[p.attributes.name].remove(new Graphic(symbol, p));
      ...
      _clusterGraphics: function () {
        ...
        for (var j = 0, jl = this._clusterData.length; j < jl; j++) {
          ...
          var targetLayer = this.map.getLayer(this._clusterData[j].attributes.name);
          // targetLayer.setVisibility(targetLayer.visible);
          if (targetLayer.visible) {
            this._clearGraphics(point);
            var clustered = false;
      ...
      //removing graphic from layer whenever it was going to be part of cluster
      _clearGraphics: function (c) {
        //getting layer from  map
        var gl = this.map.getLayer(c.attributes.name);
        if (gl) {
          gl.clear();   //clearing graphic on  layer
        }
      },
      //adding graphic to the layer
      _addGraphicToCluster: function (c) {
        var symbol = new SimpleMarkerSymbol({
          "color": new Color(c.attributes.color),
          "size": 12,
          "angle": -30,
          "xoffset": 0,
          "yoffset": 0,
        ...
        this._layerArray[c.attributes.name].add(new Graphic(new Point(c.x, c.y, this._sr), symbol, c));
        var gl = this.map.getLayer(c.attributes.name);
        if (gl) {
          gl.add(new Graphic(new Point(c.x, c.y, this._sr), symbol, c));   //adding graphic to layer
        }
      ...
      _addSingles: function (singles) {
        ...
        arrayUtils.forEach(singles, function (p) {
          var targetLayer = this.map.getLayer(p.attributes.name);
          // targetLayer.setVisibility(targetLayer.visible);
          if (targetLayer.visible) {
            var point = new Point(p.x, p.y, this._sr);
            var g = new Graphic(
              point,
              this._singleSym,
              p.attributes,
